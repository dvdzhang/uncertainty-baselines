...
class GraphAugment:
  ...
  def augment(self, input_graph: Dict[str, tf.Tensor]) -> Dict[str, tf.Tensor]:
    ...
    augmented_graph, _ = self.augmentations_available[aug_function_name](
        input_graph)
    augmented_graph['dist2topk_nbs'] = input_graph['dist2topk_nbs']
    augmented_graph['molecule_id'] = input_graph['molecule_id']
  ...
  def permute_edges(
    ...
    def _permute_edges_helper(
      ...
      # 60 - int(60*aug_ratio) and permute only those indices, leaving out the
      # rest.
